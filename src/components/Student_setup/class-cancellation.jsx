...
import "./class-cancellation.css"

const types = ["All", "Class", "Demo"]
const slotStatuses = ["All", "Slot Deleted", "Rescheduled"]

function toCsv(rows) {
  const headers = ["Slot Id", "Type", "Teacher", "Course", "Class Date & Time", "Slot Status", "Reason"]
  const escape = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`
  const body = rows
    .map((r) =>
      [r.slotId, r.type, r.teacher, r.course, r.dateTime, r.slotStatus, r.reason || "-"].map(escape).join(","),
    )
    .join("\n")
...
function makeDummyRow(i) {
  ...
  const teacherPool = ["Ms. Sarah", "Mr. David", "Ms. Lisa", "Mr. John", "Ms. Anna"]
...
export default function ClassCancellationReport() {
  ...
  const onDownload = () => {
    ...
    download("my-class-cancellation-report.csv", csv)
  }

  return (
    <>
      <div className="content-header">
        <h1>MY CLASS CANCELLATION REPORT</h1>
      </div>
      ...
      <div className="cc-controls">
        <div className="cc-left">
          ...
          <div className="cc-field cc-grow">
            ...
            <input
              type="text"
              placeholder="Slot Id, teacher, course, reason..."
              value={keyword}
